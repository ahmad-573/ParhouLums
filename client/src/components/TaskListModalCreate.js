...
function ModalCreateTask (props) {
    ...
    const addTodo = async (e) => {
        e.preventDefault();
        console.log(title, date, assigned, description, props.mList, props.mMap)
        const [data, err] = await apiInvoker('/api/createTask', {category: props.category, title: title, description: description, group_id: props.group_id, deadline: date, assign_to: 0})
                    ...
                    <div className="relative bg-white rounded-lg shadow bg-[#FFFFFF]" onClick={e => e.stopPropagation()}>
                        ...
                        <form  onSubmit={addTodo} className="px-6 pb-4 space-y-3 lg:px-8 sm:pb-6 xl:pb-8" action="#">
                            ...
                            <div>
                                ...
                                <input onChange={(e) => setTitle(e.target.value)} type="text" name="title" id="title" className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" placeholder="Enter title here" required/>
                            </div>
                            <div>
                                <label for="details" className="block mb-2 text-sm font-medium text-[#1D1C1D]">Details</label>
                                <div className='grid grid-cols-2 gap-4'>
                                    <input onChange={(e) => setDate(e.target.value)} type="date" name="date" id="date" className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" required/>
                                    <select onChange={(e) => setAssigned(e.target.value)} name="assigned" id="assigned" className='bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5'>
                            ...
                            <div>
                                ...
                                <textarea onChange={(e) => setDescription(e.target.value)} type="text" name="description" id="description" className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" placeholder="Enter description here" required/>
                            </div>
